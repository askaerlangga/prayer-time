...
class PreferencesWindow(Adw.PreferencesWindow):
    def __init__(self, parent):
        ...
        for api_name, key, default in self.prayers:
            # Modern Adw.SpinRow for numerical preferences
            val = settings.get_setting(key, default)
            adj = Gtk.Adjustment(value=val, lower=0, upper=60, step_increment=1, page_increment=5)
            row = Adw.SpinRow(adjustment=adj, climb_rate=1, digits=0)
            row.connect("changed", self.on_iqamah_changed, key)
    ...
    def on_iqamah_changed(self, spin_row, key):
        settings.set_setting(key, int(spin_row.get_value()))
